...
| 语言 | 框架     | 数据库 | 查询语句 | 纳秒/每操作（越低越好） | 查询数/秒Qps(越高越好) |内存（越低越好） |
| ------ | ------ | ------ |------ |------ |------ |------ |
| Rust   | rbatis       | mysql(docker with 1CPU,1G Mem)    | select count(1) from table; 10000次    | 965649 ns/op   |  1035  |  2.1MB   |      
| Golang | GoMybatis    | mysql(docker with 1CPU,1G Mem)    | select count(1) from table; 10000次    | 1184503 ns/op  |  844   |  28.4MB  |     
...
rbatis-core = { version = "1.2.6",  default-features = false , features = ["all","runtime-async-std"]}
rbatis =  { version = "1.2.6" } 
```
