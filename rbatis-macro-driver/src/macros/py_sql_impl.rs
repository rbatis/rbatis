       ...
       pub async fn #func_name_ident(#func_args_stream) -> #return_ty {
         ...
         match driver_type{
            rbatis::DriverType::Postgres => {
                #[rb_py(#sql_ident,'$')]
                pub fn #func_name_ident(arg: &mut bson::Bson) {}
                let (mut sql,rb_args) = #func_name_ident(&mut bson::Bson::Document(rb_arg_map));
                #call_method
            }
...
fn filter_args_context_id(
    ...
    for item in fn_arg_name_vec {
        ...
        sql_args_gen = quote! {
             #sql_args_gen
             rb_arg_map.insert(#item_ident_name.to_string(), bson::to_bson(#item_ident).unwrap());
        };
    }
    sql_args_gen
}
