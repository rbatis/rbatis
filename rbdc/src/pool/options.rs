...
impl PoolOptions {
    ...
    where
        for<'c> F: Fn(&'c mut Box<dyn Connection>) -> BoxFuture<'c, Result<(), Error>>
            + 'static
            + Send
            + Sync,
    {
    ...
    where
        for<'c> F: Fn(&'c mut Box<dyn Connection>) -> BoxFuture<'c, Result<bool, Error>>
            + 'static
            + Send
            + Sync,
    {
    ...
    where
        F: Fn(&mut Box<dyn Connection>) -> bool + 'static + Send + Sync,
    {
    ...
    pub async fn connect(self, driver: Box<dyn Driver>, uri: &str) -> Result<Pool, Error> {
        self.connect_with(driver.make_option(uri)?).await
    }

    /// Creates a new pool from this configuration and immediately establishes one connection.
    pub async fn connect_with(self, options: Box<dyn ConnectOptions>) -> Result<Pool, Error> {
    ...
    pub fn connect_lazy(self, driver: Box<dyn Driver>, uri: &str) -> Result<Pool, Error> {
        Ok(self.connect_lazy_with(driver.make_option(uri)?))
    }

    /// Creates a new pool from this configuration and will establish a connections as the pool
    /// starts to be used.
    pub fn connect_lazy_with(self, options: Box<dyn ConnectOptions>) -> Pool {
        let shared = SharedPool::new_arc(self, options);
...
async fn init_min_connections(pool: &Arc<SharedPool>) -> Result<(), Error> {
    for _ in 0..std::cmp::max(pool.options.min_connections, 1) {
