...
fn test_set_node_collection() {
    let sql = "set collection = 'collection', skips= 'id':
  if name != null:
     name = #{name},
  if age != null:
     age = #{age}";
...
for item in items:
  ...
  if item == null:
    ...
    // 验证至少包含以下节点类型
    let has_string = nodes
        .iter()
        .any(|node| matches!(node, NodeType::NString(_)));
    let has_where = nodes.iter().any(|node| matches!(node, NodeType::NWhere(_)));
    let has_choose = nodes
        .iter()
        .any(|node| matches!(node, NodeType::NChoose(_)));
    let has_set = nodes.iter().any(|node| matches!(node, NodeType::NSet(_)));
    let has_foreach = nodes
        .iter()
...
fn test_syntax_errors() {
    ...
    assert!(
        result.is_err(),
        "Expected error for missing 'in' keyword in for loop"
    );
    ...
    assert!(
        result.is_err(),
        "Expected error for missing parameter in trim statement"
    );
