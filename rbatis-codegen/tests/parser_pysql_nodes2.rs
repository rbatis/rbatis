...
fn test_parse_bind_node() {
    ...
    match &nodes[0] {
        NodeType::NForEach(node) => match &node.childs[0] {
...
fn test_parse_break_node() {
    ...
    match &nodes[0] {
        NodeType::NForEach(node) => match &node.childs[0] {
            NodeType::NIf(if_node) => match &if_node.childs[0] {
                NodeType::NBreak(_) => {}
...
fn test_parse_complex_structure() {
    ...
    // 验证有 SqlNode
    let has_sql_node = nodes.iter().any(|node| matches!(node, NodeType::NSql(_)));
    assert!(
        has_sql_node,
        "Expected to find a SqlNode in the parsed result"
    );

    // 验证有 WhereNode
    let has_where_node = nodes.iter().any(|node| matches!(node, NodeType::NWhere(_)));
    assert!(
        has_where_node,
        "Expected to find a WhereNode in the parsed result"
    );

    // 验证有 ForEachNode
    let has_foreach_node = nodes
        .iter()
        .any(|node| matches!(node, NodeType::NForEach(_)));
    assert!(
        has_foreach_node,
        "Expected to find a ForEachNode in the parsed result"
    );
}
