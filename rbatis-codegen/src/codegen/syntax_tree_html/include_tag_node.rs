        ...
        FChildParser: FnMut(&[Element], &mut TokenStream, &mut Vec<String>, &str) -> TokenStream,
    {
        ...
        let resolved_element = match url.scheme() {
            "file" => {
                ...
                let fragment_ref_id = url
                    .query_pairs()
                    .find(|(k, _)| k == "refid")
                    .map(|(_, v)| v.to_string())
                    .unwrap_or_else(|| panic!("No ref_id found in URL {}", self.refid));
                let mut file = File::open(&file_path).unwrap_or_else(|_| {
                    panic!(
                        "[rbatis-codegen] can't find file='{}', url='{}'",
                        file_path.to_str().unwrap_or_default(),
                ...
                file.read_to_string(&mut html).expect("Failed to read file");
                Self::load_mapper_vec(&html)
                    .expect("Failed to parse HTML from included file")
                    .into_iter()
                    .find(|e| {
                        e.tag == SqlTagNode::node_tag_name()
                            && e.attrs.get("id") == Some(&fragment_ref_id)
                    })
                    .unwrap_or_else(|| {
                        panic!(
                            "No ref_id={} found in file={}",
