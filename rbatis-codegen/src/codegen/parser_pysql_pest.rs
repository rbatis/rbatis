...
#[cfg(feature = "use_pest")]
#[derive(Parser)]
#[grammar = "src/codegen/pysql.pest"]
struct PySqlParser;
...
pub fn impl_fn_py(m: &ItemFn, args: &ParseArgs) -> TokenStream {
    ...
    #[cfg(feature = "use_pest")]
    // 使用Pest解析器解析py_sql语法
    let nodes = parse_pysql(&data).expect("[rbatis-codegen] parse py_sql fail!");
...
#[cfg(feature = "use_pest")]
/// 使用Pest解析器解析py_sql语法，完全遵循原始parser_pysql的逻辑
pub fn parse_pysql(arg: &str) -> Result<Vec<NodeType>, Error> {
    let line_space_map = create_line_space_map(&arg);
    ...
    for x in ls {
        ...
        let (child_str, do_skip) =
            find_child_str(line, count_index, arg, &line_space_map);
        if do_skip != -1 && do_skip >= skip {
            skip = do_skip;
        }
        let parserd;
        if !child_str.is_empty() {
            parserd = parse_pysql(child_str.as_str())?;
        } else {
            parserd = vec![];
        }
        parse_pysql_node(
...
#[cfg(feature = "use_pest")]
/// 处理字符串文本节点，附加额外处理以修复SQL格式问题
fn process_string_node(value: &str) -> String {
...
#[cfg(feature = "use_pest")]
/// 解析py_sql节点，遵循原始parser_pysql的逻辑
fn parse_pysql_node(
    ...
    if trim_x.ends_with(":") {
        ...
        if trim_x.contains(": ") {
            ...
            if vecs.len() > 1 {
                ...
                for index in 0..len {
                    ...
                    let item = vecs[index];
                    childs = vec![parse_trim_node(item, x, childs)?];
                    if index == 0 {
                        for x in &childs {
        ...
        let node = parse_trim_node(trim_x, x, childs)?;
        main_node.push(node);
        return Ok(());
    } else {
        ...
        // 处理特殊情况 - tablewhere 连接问题和反引号SQL
        data = process_string_node(&data);
