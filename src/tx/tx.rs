...
/// This object contains a small amount of unsafe (but check if conn is null)
/// to avoid the life cycle (because if the life cycle is used, then & mut can only be used once in this cycle, which violates the original requirement that a session can start multiple transactions)
/// Tx即事务抽象
