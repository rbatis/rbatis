...
#[test]
fn test_parser() {
    let box_node = parser::parser(String::from("a == 1 && a != 0"), &OptMap::new()).unwrap();
    let john = json!({
        "a":1,
        "name": "John Doe",
        "age": {
...
#[test]
fn test_benchmark() {
    let box_node = parser::parser(String::from("1<=2"), &OptMap::new()).unwrap();
    let john = json!({
        "name": "John Doe",
    });
    let total=10000000;
    ...
    utils::time_util::count_each_time("test_benchmark",total, now);
    utils::time_util::count_tps("test_benchmark",total,now);
