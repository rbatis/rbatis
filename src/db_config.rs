...
impl DBConfig {
    pub fn new(link: String) -> Result<DBConfig, rbatis_core::Error> {
        if link.find("://").is_none() {
            return Result::Err(rbatis_core::Error::from("[rbatis] link must have [type]://[user]:[password]@[ip]:[port]/[db_name], miss ://"));
        }
        ...
        if user_pwd_link[0].find(":").is_none() {
            return Result::Err(rbatis_core::Error::from("[rbatis] link must have [type]://[user]:[password]@[ip]:[port]/[db_name], miss ':' of [user]:[password]"));
        }
        let user_pwd: Vec<&str> = user_pwd_link[0].split(":").collect();

        if user_pwd_link[1].find("/").is_none() {
            return Result::Err(rbatis_core::Error::from("[rbatis] link must have [type]://[user]:[password]@[ip]:[port]/[db_name], miss '/' of [port]/[db_name]"));
        }
        let link_dbname: Vec<&str> = user_pwd_link[1].split("/").collect();

        if link_dbname[0].find(":").is_none() {
            return Result::Err(rbatis_core::Error::from("[rbatis] link must have [type]://[user]:[password]@[ip]:[port]/[db_name], miss ':' of [ip]:[port]"));
        }
