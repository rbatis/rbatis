...
pub trait CRUDTable: Send + Sync + Serialize {
    ...
    fn make_value_sql_arg(
        ...
        index: &mut usize,
        skips: &[Skip],
    ) -> Result<(String, String, Vec<bson::Bson>)> {
...
impl<Table> Fields for Vec<Table> where Table: CRUDTable {
    fn to_fields<T>(&self, column: &str) -> Vec<T> where T: DeserializeOwned {
        ...
        let d = bson::from_bson(bson::Bson::Array(results));
        match d {
            Ok(v) => {
                return v;
            }
...
pub trait CRUDMut: ExecutorMut {
    ...
    /// update_by_wrapper
    /// skips: use &[Skip::Value(&bson::Bson::Null), Skip::Column("id"), Skip::Column(column)] will skip id column and null value param
    async fn update_by_wrapper<T>(
        &mut self,
        table: &T,
        w: Wrapper,
            ...
            T: CRUDTable,
    {
        let table_name = choose_dyn_table_name::<T>(&w);
        let mut args = vec![];
        let mut old_version = bson::Bson::Null;
        let driver_type = &self.driver_type()?;
