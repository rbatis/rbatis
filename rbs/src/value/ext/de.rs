...
impl<'de> de::VariantAccess<'de> for VariantDeserializer {
    ...
    fn unit_variant(self) -> Result<(), Error> {
        match self.value {
            ...
            None => Err(de::Error::invalid_value(Unexpected::Other(&format!("none")), &"not support")),
        }
    }
            ...
            T: de::DeserializeSeed<'de>,
    {
        match self.value {
            Some(v) => {
                let m = v.into_map();
                if m.is_none() {
                    return Err(de::Error::custom(format!("Deserialize newtype_variant must be {}, and len = 1", "{\"key\",\"v\"}")));
                }
                let m = m.unwrap();
                if m.len() != 1 {
                    return Err(de::Error::custom(format!("Deserialize newtype_variant must be {}, and len = 1", "{\"key\",\"v\"}")));
...
trait ValueBase<'de>: Deserializer<'de, Error=Error> + ValueExt where Self: 'static {
            ...
            V: Visitor<'de>,
    {
        ...
        let v = match v {
            Value::String(v) => {
                visitor.visit_enum(EnumDeserializer { variant: v.clone(), value: Some(Value::String(v)) })
            }
            Value::Map(m) => {
                ...
                visitor.visit_enum(EnumDeserializer { variant: m.0[0].0.clone().into_string().unwrap(), value: Some(Value::Map(m)) })
            }
            _ => {
                return Err(de::Error::invalid_type(
